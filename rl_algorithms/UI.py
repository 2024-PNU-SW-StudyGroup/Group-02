...
class GridWorldViz:
    ...
    def handle_algo_selection(self, pos):
        ...
        for i, button in enumerate(self.algo_buttons):
            if button['rect'].collidepoint(pos):
                ...
                # 알고리즘 변경 통지
                self.notify_observers('algorithm_changed', {
    ...
    def handle_viz_config(self, pos):
        ...
        for i, button in enumerate(self.viz_config_buttons):
            if button['rect'].collidepoint(pos):
                ...
                # 시각화 설정 변경 통지
                self.notify_observers('visualization_changed', {
                    'type': button['text'],
                    'state': button['state']
                })
    ...
    def move_agent(self):
        ...
        if self.env.is_terminated():
            ...
            # 에이전트 이동 통지
            self.notify_observers('agent_moved', {
